...
import SymptomsSettingsPage from "./symptoms";
		...
		<main class=" w-full h-svh flex flex-col bg-cycle-secondary px-4 overflow-hidden">
			...
			<div class="flex-1 rounded-2xl overflow-hidden mb-4">
				<div class="h-full overflow-y-auto no-scrollbar flex flex-col gap-4">
					...
					<Settings.Group>
						<Settings.Tab
							onClick={() => navigator.overlay(ProfileSettingsPage)}
							class="flex flex-row gap-4"
						>
							<Settings.TabIcon
								class="bg-period-primary"
								icon={FaSolidUser}
							/>
					...
					<Settings.Group>
						<Settings.Tab
							onClick={() => navigator.overlay(SymptomsSettingsPage)}
							class="flex flex-row gap-4"
						>
							<Settings.TabIcon
								class="bg-period-primary"
						...
						<Settings.Tab onClick={() => navigator.overlay(SecuritySettingsPage)}>
							<Settings.TabIcon
								class="bg-luteal-primary"
								icon={FaSolidShield}
							/>
			...
			<button type="button" onClick={() => context.setSettings("setup_complete", false)}>
				Reset
			</button>
		</main>
	);
