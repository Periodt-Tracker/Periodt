...
const PeriodLength: Component = () => {
	...
	const lowerBound = () => context.form.period_length.lower;
	const upperBound = () => context.form.period_length.upper;

	const lowerRange = Array.from(_.range(1, 9, formatWheelItems));
	const upperRange = Array.from(_.range(2, 10, formatWheelItems));

	const setLowerBound = (value: number) => {
		const upper = upperBound();

		if (upper <= value) {
			context.setForm("period_length", "upper", value + 1);
		}

		context.setForm("period_length", "lower", value);
	};
	...
	const setUpperBound = (value: number) => {
		...
		if (lower >= value) {
			context.setForm("period_length", "lower", value - 1);
		}

		context.setForm("period_length", "upper", value);
	};

	return (
		<section class="flex flex-col">
			<h2 class="text-3xl font-semibold">And how long is your usual period?</h2>
			...
			<Dialog>
				...
				<DialogContent>
					<DialogTitle>Not sure how long your period is?</DialogTitle>
					...
					<DialogFooter class="flex flex-col gap-4">
						...
						<CloseButton as={Button} class="bg-gray-400">
							I'll pick
						</CloseButton>
					</DialogFooter>
				</DialogContent>
