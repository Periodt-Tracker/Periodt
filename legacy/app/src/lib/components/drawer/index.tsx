	...
	return (
		<DrawerPortal>
			...
			<DrawerPrimitive.Content
				class={merge(
					"fixed inset-x-0 rounded-t-4xl bottom-0 z-50 mt-24 flex h-auto flex-col border bg-background after:absolute after:inset-x-0 after:top-full after:h-1/2 after:bg-inherit data-[transitioning]:transition-transform data-[transitioning]:duration-300 md:select-none",
					props.class,
				)}
				{...rest}
			>
...
const DrawerPage = <T extends ValidComponent = "div">(
	props: DynamicProps<T, DrawerContentProps<T>>,
) => {
	const drawerContext = DrawerPrimitive.useContext();
	...
	return (
		<DrawerPortal>
			...
			<DrawerPrimitive.Content
				class={merge(
					"fixed inset-x-0 bottom-0 z-50 mt-24 flex h-auto flex-col border bg-background after:absolute after:inset-x-0 after:top-full after:h-1/2 after:bg-inherit data-[transitioning]:transition-transform data-[transitioning]:duration-300 md:select-none",
					props.class,
				)}
				style={{
					"border-top-left-radius": `${4 * (1 - drawerContext.openPercentage())}rem`,
					"border-top-right-radius": `${4 * (1 - drawerContext.openPercentage())}rem`,
				}}
				{...rest}
			>
				<div class="mx-auto mt-4 h-2 w-25 rounded-full bg-muted" />
...
const DrawerHeader: Component<ComponentProps<"div">> = (props) => {
	...
	return (
		<div
			class={merge("grid gap-1.5 p-4 text-center sm:text-left", props.class)}
			{...rest}
		/>
	);
};
...
const DrawerFooter: Component<ComponentProps<"div">> = (props) => {
	...
	return (
		<div
			class={merge("t-auto flex flex-col gap-2 p-4", props.class)}
			{...rest}
		/>
	);
};
