...
const SelectTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <SelectPrimitive.Trigger
      class={merge(
        "flex h-10 w-full items-center justify-between rounded-md border-2 border-base-300 px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 outline-brand-100",
        local.class,
      )}
      {...others}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={merge(
          "relative z-50 min-w-32 overflow-hidden rounded-md border-2 border-base-300 bg-base-100 shadow-md animate-in fade-in-80",
          local.class,
        )}
        {...others}
      >
...
const SelectLabel = <T extends ValidComponent = "label">(
  ...
  return (
    <SelectPrimitive.Label
      class={merge(
        "text-sm font-medium text-accent-50 leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 data-[invalid]:text-destructive",
        local.class,
      )}
      {...others}
    />
...
const SelectDescription = <T extends ValidComponent = "div">(
  ...
  return (
    <SelectPrimitive.Description
      class={merge(
        "text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-accent-50",
        local.class,
      )}
      {...others}
    />
...
const SelectErrorMessage = <T extends ValidComponent = "div">(
  ...
  return (
    <SelectPrimitive.ErrorMessage
      class={merge(
        "text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-red-500",
        local.class,
      )}
      {...others}
    />
