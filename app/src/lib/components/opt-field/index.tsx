...
const OTPFieldGroup: Component<ComponentProps<"div">> = (props) => {
	...
	return (
		<div class={merge("flex items-center gap-2", local.class)} {...others} />
	);
};
...
const OTPFieldSlot: Component<ComponentProps<"div"> & { index: number }> = (
	...
	return (
		<div
			class={merge(
				"group relative flex size-12 h-14 items-center justify-center bg-cycle-light text-sm rounded-xl shadow-md",
				local.class,
			)}
			{...others}
		>
			<div
				class={merge(
					"absolute inset-0 z-10 transition-all rounded-xl",
					context.activeSlots().includes(local.index) &&
						"ring-2 ring-cycle-primary ring-offset-background",
				)}
			/>
			{char()}
